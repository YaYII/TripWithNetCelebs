...
interface PageData {
  ...
  tabs: string[];
  currentTab: number;
  sortType: 'recommend' | 'price' | 'popularity';
  priceSort: 'asc' | 'desc';
  currentCity: string;
  page: number;
  hasMore: boolean;
...
interface PageMethods {
  ...
  openFilter: () => void;
  closeFilter: () => void;
  setPriceRange: (e: WechatMiniprogram.TouchEvent) => void;
  setDurationRange: (e: WechatMiniprogram.TouchEvent) => void;
  resetFilter: () => void;
  applyFilter: () => void;
}
...
Page<PageData, PageMethods>({
  ...
  // 设置价格区间
  setPriceRange(e: WechatMiniprogram.TouchEvent) {
    const range = e.currentTarget.dataset.range as string;
    this.setData({
      priceRange: range
    });
  },
  ...
  // 设置行程天数
  setDurationRange(e: WechatMiniprogram.TouchEvent) {
    const range = e.currentTarget.dataset.range as string;
    this.setData({
      durationRange: range
    });
